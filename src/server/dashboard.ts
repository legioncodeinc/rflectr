...
const DASHBOARD_CSS = `
:root{color-scheme:dark;--bg:#090a0d;--surface:#111319;--elev:#171a22;--border:#2a2f3a;--text:#f5f2e8;--muted:#9ca3af;--relay:#f0b429;--ok:#49d17d;--warn:#f2a23c;--bad:#ff5c7a;--tool:#8ab4ff}html[data-theme=light]{color-scheme:light;--bg:#f7f7f5;--surface:#fff;--elev:#f0f1f4;--border:#d8dbe2;--text:#111319;--muted:#606774}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px Inter,ui-sans-serif,system-ui,sans-serif}button,input,select{font:inherit}.app{display:flex;min-height:100vh}.sidebar{width:230px;background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;transition:width .16s ease}.sidebar.collapsed{width:64px}.brand{height:60px;display:flex;align-items:center;gap:10px;padding:0 18px;border-bottom:1px solid var(--border);font-weight:800;font-size:21px}.mark{width:26px;height:26px;border-radius:8px;background:linear-gradient(135deg,var(--relay),#9ff0d0)}nav{display:flex;flex-direction:column;gap:4px;padding:12px;flex:1}.nav{height:38px;border:1px solid transparent;border-radius:8px;background:transparent;color:var(--muted);display:flex;align-items:center;gap:11px;padding:0 12px;cursor:pointer;text-align:left}.nav.active{border-color:rgba(240,180,41,.5);background:rgba(240,180,41,.12);color:var(--relay)}.svgicon{width:16px;height:16px;display:inline-block;flex:0 0 16px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.collapsed .nav{justify-content:center;padding:0}.collapsed .label,.collapsed .word,.collapsed .user-meta,.collapsed .local-text{display:none}.side-foot{padding:12px;border-top:1px solid var(--border);display:flex;flex-direction:column;gap:10px}.local{border:1px solid rgba(73,209,125,.4);background:rgba(73,209,125,.1);color:var(--ok);border-radius:8px;padding:8px 10px;font:11px ui-monospace,monospace}.main{flex:1;min-width:0;display:flex;flex-direction:column}.top{height:60px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:14px;padding:0 24px;background:color-mix(in oklab,var(--bg) 86%,transparent);backdrop-filter:blur(8px);position:sticky;top:0;z-index:5}.title{font-weight:700;font-size:17px}.grow{flex:1}.iconbtn,.btn,.seg{border:1px solid var(--border);background:transparent;color:var(--text);height:34px;border-radius:8px;padding:0 12px;cursor:pointer}.iconbtn{display:inline-flex;align-items:center;justify-content:center}.btn.primary{background:var(--relay);color:#16120a;border-color:var(--relay);font-weight:700}.btn.danger{border-color:rgba(255,92,122,.55);color:var(--bad)}.btn:disabled,.seg:disabled{opacity:.55;cursor:not-allowed}.search,.field input,.field select{height:36px;border:1px solid var(--border);border-radius:8px;background:var(--elev);color:var(--text);padding:0 12px}.search{width:230px}.content{max-width:1200px;margin:0 auto;padding:28px 32px 64px;width:100%}.grid4{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}.card{background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:16px}.kpi .value,.value{font:600 26px ui-monospace,monospace}.eyebrow{font:700 11px ui-monospace,monospace;text-transform:uppercase;letter-spacing:.08em;color:var(--muted)}.routes{display:grid;grid-template-columns:1fr 1fr;gap:14px}.route{display:grid;grid-template-columns:1fr auto 1fr;gap:10px;align-items:center}.mono{font-family:ui-monospace,monospace}.pill{border-radius:999px;padding:3px 8px;font:11px ui-monospace,monospace}.ok,.success,.connected,.localstatus{color:var(--ok)}.retry,.warn,.oauth{color:var(--warn)}.error,.bad,.missing,.unsupported{color:var(--bad)}.ready,.native,.translated{color:var(--ok)}.table{width:100%;border-collapse:collapse}.table th{color:var(--muted);font:10px ui-monospace,monospace;text-transform:uppercase;text-align:left}.table td,.table th{padding:9px 10px;border-bottom:1px solid rgba(128,128,128,.12)}.providers{display:grid;grid-template-columns:repeat(auto-fill,minmax(248px,1fr));gap:14px}.provider{text-align:left}.provider.selected{outline:1px solid var(--relay)}.modelrow{display:grid;grid-template-columns:2fr 1fr .8fr .7fr .8fr .8fr auto;gap:10px;align-items:center;margin-bottom:8px}.toolbar{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:16px}.seg.active{border-color:var(--relay);color:var(--relay);background:rgba(240,180,41,.12)}.empty{text-align:center;color:var(--muted);padding:32px}.notice{background:rgba(242,162,60,.12);border:1px solid rgba(242,162,60,.35);border-radius:8px;padding:12px 14px;color:var(--muted)}.form{display:grid;gap:14px;margin-bottom:18px}.field{display:grid;gap:6px}.field label{color:var(--muted);font:11px ui-monospace,monospace;text-transform:uppercase}.field input,.field select{width:100%}.field input[type=checkbox]{width:16px;height:16px;padding:0;vertical-align:middle}.checklabel{display:flex;align-items:center;gap:8px;text-transform:none!important;font:14px ui-monospace,monospace!important;color:var(--text)!important}.formgrid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.form-actions{display:flex;gap:8px;justify-content:flex-end}.statusline{min-height:18px;color:var(--muted)}@media(max-width:900px){.grid4{grid-template-columns:1fr 1fr}.routes{grid-template-columns:1fr}.search{display:none}.modelrow,.formgrid{grid-template-columns:1fr}.hide-sm{display:none}}
`;
...
let DATA=null,route=safeGet('rflectr-dashboard-route')||'overview',collapsed=safeGet('rflectr-dashboard-collapsed')==='1',query='',modelFilter='all',activityFilter='all',importBusy=false,saveBusy=false,providerForm=null,providerTemplates=null,providerBusy=false,providerStatus='',desktopBusy=false,desktopStatus='';
...
function shell(body){const g=DATA.settings.gateway,sourceMeta=DATA.settings.modelSourceAvailable?'<div><b>RF</b> <span class="user-meta">'+sourceLabel(DATA.settings.modelSource)+'</span></div>':'';app.className='app';app.innerHTML='<aside class="sidebar '+(collapsed?'collapsed':'')+'"><div class="brand"><span class="mark"></span><span class="word">rflectr</span></div><nav>'+nav('overview','Overview','overview')+nav('providers','Providers','providers')+nav('models','Models','models')+nav('activity','Activity','activity')+nav('settings','Settings','settings')+'</nav><div class="side-foot"><div class="local"><span>●</span> <span class="local-text">'+esc(g.address.replace(/^https?:\\/\\//,''))+'</span></div>'+sourceMeta+'</div></aside><main class="main"><header class="top"><button class="iconbtn" title="Toggle sidebar" onclick="toggleSide()">'+icon('menu')+'</button><div class="title">'+title()+'</div><div class="grow"></div><input class="search" placeholder="Search models, providers" value="'+esc(query)+'" oninput="query=this.value;render()"><button class="iconbtn" title="Toggle theme" aria-label="Toggle theme" onclick="toggleTheme()">'+icon('theme')+'</button><button class="btn primary" onclick="addProvider()">+ Add provider</button></header><div class="content">'+body+'</div></main>'}
function title(){return ({overview:'Overview',providers:'Providers',models:'Models',activity:'Activity',settings:'Settings'})[route]||'Overview'}function go(id){route=id;safeSet('rflectr-dashboard-route',id);render()}function toggleSide(){collapsed=!collapsed;safeSet('rflectr-dashboard-collapsed',collapsed?'1':'0');render()}function toggleTheme(){const l=document.documentElement.getAttribute('data-theme')==='light';if(l)document.documentElement.removeAttribute('data-theme');else document.documentElement.setAttribute('data-theme','light');safeSet('rflectr-dashboard-theme',l?'dark':'light')}
...
function settings(){const s=DATA.settings,g=s.gateway,d=s.claudeDesktop,sourceCard=s.modelSourceAvailable?'<div class="card"><h3>OpenCode model source</h3><div class="toolbar">'+['free','zen','go'].map(t=>'<button class="seg '+(s.modelSource===t?'active':'')+'" onclick="saveSettings({modelSource:\\''+t+'\\'})">'+sourceLabel(t)+'</button>').join('')+'</div></div>':'';const desktopCard='<div class="card"><h3>Claude Desktop</h3><p>Gateway <span class="mono">'+esc(d.baseUrl)+'</span></p><p>Auth <span class="mono">'+esc(d.authScheme)+'</span> · '+esc(d.apiKeyLabel)+'</p><p class="mono">Use Cowork or Code. Chat is not available in gateway mode.</p><div class="toolbar"><button class="btn primary" onclick="connectClaudeDesktop(true)" '+(!d.supported||desktopBusy?'disabled':'')+'>'+(desktopBusy?'Configuring...':'Configure & open')+'</button><button class="btn" onclick="connectClaudeDesktop(false)" '+(!d.supported||desktopBusy?'disabled':'')+'>Configure only</button></div><div class="statusline">'+esc(desktopStatus||(!d.supported?'Claude Desktop gateway setup is supported on macOS and Windows only.':'Writes Claude Desktop third-party inference config for this running gateway.'))+'</div></div>';return '<div style="max-width:720px;display:flex;flex-direction:column;gap:16px">'+desktopCard+sourceCard+'<div class="card"><h3>Default tool</h3><div class="toolbar">'+[['claude','Claude Code',true],['codex','Codex',true],['gemini','Gemini CLI',true],['cursor','Cursor',false]].map(t=>'<button '+(!t[2]?'disabled title="Cursor launch is not available from bare rflectr yet"':'')+' class="seg '+(s.defaultTool===t[0]?'active':'')+'" onclick="saveSettings({defaultTool:\\''+t[0]+'\\'})">'+esc(t[1])+'</button>').join('')+'</div></div><div class="card"><h3>Routing</h3><label><input type="checkbox" '+(s.routing.autoReroute?'checked':'disabled')+'> Auto-reroute on error</label><hr><label><input type="checkbox" '+(s.routing.requestTracing?'checked':'')+' onchange="saveSettings({requestTracing:this.checked})"> Request tracing</label><p class="mono">Traces stay local under ~/.rflectr/logs when enabled.</p></div><div class="card"><h3>Gateway</h3><p>Address <span class="mono">'+esc(g.address)+'</span></p><p>Version <span class="mono">rflectr '+esc(g.version)+'</span></p><p>Uptime <span class="mono">'+Math.floor(g.uptimeSeconds/60)+'m</span></p><p class="ok">● runs on your machine</p><button class="btn" onclick="restart()" '+(!g.restartSupported?'disabled':'')+'>Restart gateway</button></div></div>'}
