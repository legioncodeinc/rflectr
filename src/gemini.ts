...
// src/gemini.ts — rflectr gemini: launch Google Gemini CLI with registry providers
...
${pc.bold('Description:')}
  Picks a provider and model from ~/.rflectr/providers.json, starts a local Gemini-to-SDK translation
...
${pc.bold('Passing flags to Gemini CLI:')}
  ...
  rflectr manages -m / --model and -p / --prompt; other flags go to Gemini CLI.

${pc.bold('Examples:')}
  rflectr gemini
  rflectr gemini --trace
  rflectr gemini --provider zen --model gemini-2.5-flash
  rflectr gemini -p "review this file"`;
}
...
export async function runGeminiCommand(
  ...
  if (launchPlan.skip && launchPlan.target) {
    ...
    if (!agentStdout) {
      console.log('');
      p.log.info(`Launching ${pc.bold('Gemini CLI')} with rflectr`);
    }
  ...
  if (!launchApiKey?.trim()) {
    p.log.error(
      `No API key found for ${activeProvider.name}. Set it with rflectr providers add.`,
    );
    return 1;
  }
