// src/first-run.ts — inline first-run setup for rflectr claude (never dead-end)
...
export async function runFirstRunWizard(trace = false): Promise<FirstRunResult> {
  ...
  const options: Array<{ value: string; label: string; hint: string }> = [
    ...
    },
    {
      ...
      hint: hasOpencode
        ...
        : 'Import from OpenCode or add providers via rflectr providers',
    },
  ];
  if (hasOpencode) {
    options.push({
  ...
  if (choice === 'import' || choice === 'providers') {
    ...
    if (!hasOpencode) {
      p.log.info('Run rflectr providers to add providers, then rflectr claude again.');
