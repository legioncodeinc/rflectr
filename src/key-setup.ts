...
export async function resolveOrCollectApiKey(simulate = false, trace = false): Promise<string | null> {
  ...
  const saveOptions: Array<{ value: SaveChoice; label: string; hint: string }> = (() => {
    if (isMac) {
      return [
        { value: 'keychain', label: 'Keychain only', hint: 'Key stored encrypted in Keychain; rflectr reads it automatically next time' },
    ...
    if (isWindows) {
      return [
        { value: 'credential-manager', label: 'Windows Credential Manager', hint: 'Key stored securely; rflectr reads it automatically next time' },
    ...
    if (secretServiceAvailable) {
      opts.push({ value: 'secret-service', label: 'Secret Service (GNOME Keyring / KWallet)', hint: 'Key stored securely in your desktop keyring; rflectr reads it automatically next time' });
    } else if (!simulate) {
  ...
  if (simulate) {
    ...
    if (await saveToCredentialStore(trimmedKey)) {
      try {
        const autoLoadLine = `export OPENCODE_API_KEY="$(security find-generic-password -s rflectr -a ${GLOBAL_OPENCODE_KEYRING_ACCOUNT} -w 2>/dev/null)"`;
        const existing = existsSync(path) ? readFileSync(path, 'utf8') : '';
        if (!existing.includes(autoLoadLine)) {
          appendFileSync(path, `\n# rflectr: load API key from macOS Keychain\n${autoLoadLine}\n`);
        }
