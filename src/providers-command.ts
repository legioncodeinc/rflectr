...
// src/providers-command.ts — rflectr providers command
...
export function parseProvidersArgs(args: string[]): {
  ...
  if (first === 'auth') {
    ...
    if (positional.length !== 1) {
      return { subcommand: 'auth', showHelp: false, error: 'Usage: rflectr providers auth <id> [--native|--broker]' };
    }
    return { subcommand: 'auth', showHelp: false, removeId: positional[0], authMethod };
  }
  if (first === 'remove') {
    if (rest.length === 0) return { subcommand: 'remove', showHelp: false, error: 'Usage: rflectr providers remove <id>' };
...
export function providersHelpText(): string {
  return `${pc.bold('rflectr providers')} — manage your AI providers

${pc.bold('Usage:')}
  rflectr providers
  rflectr providers add
  rflectr providers import
  rflectr providers list
  rflectr providers remove <id>
  rflectr providers refresh-models [id]
  rflectr providers auth <id> [--native|--broker]
...
export async function runProvidersImport(): Promise<number> {
  ...
  const resolveConflict = hasExisting
    ? async (ctx: ImportConflictContext): Promise<ImportConflictChoice> => {
        ...
        const choice = await p.select({
          ...
          options: [
            { value: 'keep', label: pc.cyan('Keep mine'), hint: 'Leave your current rflectr config unchanged' },
  ...
  if (result.imported.length === 0 && result.skipped.length === 0) {
    ...
    p.log.info('Add providers in OpenCode first, or use rflectr providers add.');
    return 0;
  }
  ...
  if (result.skipped.length > 0) {
    for (const s of result.skipped) {
      const reason =
        ...
        : s.reason === 'oauth-no-token' ? 'OAuth provider in OpenCode but not signed in — run rflectr providers auth'
        : s.reason === 'no-api-key' ? 'no API key in OpenCode — add key there or use rflectr providers add'
...
export async function runProvidersList(): Promise<number> {
  ...
  if (entries.length === 0) {
    p.log.info('No providers configured. Run rflectr providers add or import.');
    return 0;
  }
  ...
  detailOptions.push(
    {
      ...
      hint: provider.enabled ? 'Hide from rflectr claude picker' : 'Show in rflectr claude picker',
    },
