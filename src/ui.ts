...
export function relayIntro(section: string): void {
  p.intro(`${pc.bold(pc.cyan('Rflectr'))}${pc.bold(` — ${section}`)}`);
}
...
export function printWelcomePanel(): void {
  printPanel(pc.cyan('Welcome to rflectr'), [
    `${pc.white("Let's get you set up.")}`,
    `${pc.dim('Pick a path below — you can always add more providers later with ')}${fmtCommand('rflectr providers')}${pc.dim('.')}`,
  ]);
}
...
export function printApiKeyPanel(url: string): void {
  printPanel(pc.cyan('OpenCode API key'), [
    ...
    `${pc.dim('Paste it below — rflectr stores it in your system keychain when possible.')}`,
  ]);
}
...
export function printFavoritesOnlyPanel(): void {
  printPanel(pc.cyan('Favorites-only mode'), [
    ...
    `${pc.white('Edit with ')}${pc.cyan('rflectr models')}${pc.white('.')}`,
  ]);
}
