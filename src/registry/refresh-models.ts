    ...
    if (source === 'zen-go-api') {
      ...
      if (!apiKey) {
        return {
          ...
          name: provider.name,
          ok: false,
          reason: 'OAuth token not available — try signing in again with rflectr providers auth.',
        };
      }
      ...
      if (!keyOptional && isLikelyPlaceholderKey(effectiveKey)) {
        if (cachedModelCount(provider) > 0) {
          return skipWithCachedModels(
            ...
            + 'Add this provider again via rflectr providers add with a real key to refresh live.',
          );
        }
        return {
          id: provider.id,
          name: provider.name,
          ok: false,
          reason: 'No usable API key — add the provider via rflectr providers add with a real key.',
        };
      }
      ...
      if (fetched.error) {
        if (
          ...
          return skipWithCachedModels(
            ...
            + 'Update your API key via rflectr providers add if you need a live refresh.',
          );
        }
