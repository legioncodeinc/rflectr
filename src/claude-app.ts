...
import { writeRflectrConfig, getClaudeDesktopHome } from './claude-desktop/app-config.js';
...
${pc.bold('Usage:')}
  ...
  rflectr claude-app --trace
  rflectr claude-app --restore
  rflectr claude-app --help
  rflectr claude-app --version

${pc.bold('Options:')}
  --trace      Write proxy debug logs to ~/.rflectr/logs/
...
${pc.bold('Cleanup:')}
  ...
  After a crash: rflectr claude-app --restore
`;
}
...
export async function runClaudeAppCommand(args: string[]): Promise<number> {
  ...
  if (args.includes('--restore')) {
    recoverSession();
    console.log('Restored Claude Desktop rflectr config.');
    return 0;
  }
  ...
  if (!isTty) {
    console.error(pc.red('rflectr claude-app requires an interactive terminal.'));
    return 1;
  }

  if (isConcurrentLiveSession()) {
    console.error(pc.yellow(`Another rflectr claude-app session may be running.`));
  ...
  try {
    ...
    const uuid = writeRflectrConfig(proxyHandle.port);
