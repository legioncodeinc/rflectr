// codex-app.ts — rflectr codex-app: launch Codex desktop app with registry providers
...
export function codexAppHelpText(): string {
  return `${pc.bold('rflectr codex-app')} — launch Codex desktop app with your registry providers

${pc.bold('Usage:')}
  rflectr codex-app [options]
  rflectr codex-app --vertex
  rflectr codex-app --restore
  rflectr codex-app --config
  rflectr codex-app --help
  rflectr codex-app --version
...
${pc.bold('Description:')}
  Picks a provider and model from ~/.rflectr/providers.json, patches ~/.codex/config.toml
...
${pc.bold('Cleanup:')}
  ...
  After crash: rflectr codex-app --restore

${pc.bold('Preview (no writes):')}
  rflectr codex-app --config

  See https://github.com/legioncodeinc/rflectr/blob/main/library/knowledge/public/guides/codex.md for CLI vs app, files touched, and restore.
...
${pc.bold('Favorites:')}
  When you have saved favorites via ${pc.cyan('rflectr models')}, the Codex App
...
async function runCodexAppVertexLaunch(configOnly: boolean, trace = false): Promise<number> {
  ...
  if (configOnly) {
    ...
    console.log('');
    console.log(pc.bold(pc.cyan('  CONFIG PREVIEW — rflectr codex-app --vertex')));
    console.log('');
    ...
    console.log(pc.dim('  Run ') + pc.cyan('rflectr codex-app --vertex') + pc.dim(' to launch.'));
    console.log('');
    return 0;
  }
  ...
  try {
    ...
    printCodexAppSessionPanel({
      ...
      restoreCommand: 'rflectr codex-app --restore',
    });
...
export async function runCodexAppCommand(args: string[], opts: { vertex?: boolean } = {}): Promise<number> {
  ...
  if (!configOnly) {
    ...
    if (!sessionCheck.ok) {
      if (sessionCheck.reason === 'non_tty') {
        console.error(pc.red('rflectr codex-app requires an interactive terminal.'));
        return 1;
      }
      console.error(pc.yellow(`Another rflectr codex-app session may be running (pid ${sessionCheck.lock.pid}).`));
      console.error('Stop it with Ctrl+C in that terminal, or run rflectr codex-app --restore after it exits.');
      return 1;
    }
  }
  ...
  if (compatible.length === 0) {
    if (!configOnly) {
      ...
      p.log.info('Add a provider with rflectr providers add.');
    }
    return 0;
  }
  ...
  if (!apiKey) {
    if (!configOnly) {
      p.log.error(`No credential for ${activeProvider.name}. Run rflectr providers auth ${activeProvider.id}.`);
    }
    return 1;
  }
  ...
  try {
    ...
    if (configOnly) {
      ...
      console.log('');
      console.log(pc.bold(pc.cyan('  CONFIG PREVIEW — rflectr codex-app')));
      console.log('');
      ...
      console.log('');
      console.log(pc.dim('  No app was launched.'));
      console.log(pc.dim('  Run ') + pc.cyan('rflectr codex-app') + pc.dim(' to launch.'));
      console.log('');
