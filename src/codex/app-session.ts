...
export function restoreCodexAppOverlay(env: NodeJS.ProcessEnv = process.env): RestoreAppOverlayResult {
  ...
  if (lock && lock.pid !== process.pid && isConcurrentSession(lock)) {
    return {
      restored: false,
      liveSession: true,
      message: `Another rflectr codex-app session is running (pid ${lock.pid}). Ctrl+C it first, then run --restore.`,
    };
  }
  ...
  clearAppSessionLock(env);
  return { restored: true, message: 'Restored Codex App config and removed rflectr app files.' };
}
