...
export const CODEX_PROFILE_NAME = 'rflectr-launch';
...
export function getRflectrCodexDir(env: NodeJS.ProcessEnv = process.env): string {
  return join(getAppHome(env), 'codex');
}

export function getSessionLockPath(env: NodeJS.ProcessEnv = process.env): string {
  return join(getRflectrCodexDir(env), 'session.json');
}

export function getBackupsDir(env: NodeJS.ProcessEnv = process.env): string {
  return join(getRflectrCodexDir(env), 'backups');
}

export function getCatalogPath(providerId: string, env: NodeJS.ProcessEnv = process.env): string {
  return join(getRflectrCodexDir(env), `models-${providerId}.json`);
}

/** Files rflectr owns for Codex launch — never touch ~/.codex/config.toml. */
export function ownedOverlayPaths(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const codexDir = getRflectrCodexDir(env);
...
export function writeSessionLock(lock: CodexSessionLock, env: NodeJS.ProcessEnv = process.env): void {
  const path = getSessionLockPath(env);
  mkdirSync(getRflectrCodexDir(env), { recursive: true });
...
export type InterruptedRecovery = {
  recovered: boolean;
  removedCount?: number;
  /** dead rflectr/codex pid or orphan overlay without a live lock */
