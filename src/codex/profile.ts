...
import { CODEX_PROFILE_NAME, getCatalogPath, getCodexProfilePath, getRflectrCodexDir } from './session.js';
...
export function buildCodexProfileToml(spec: CodexProfileSpec): string {
  ...
  if (route.tier === 'direct') {
    ...
    return `# Generated by rflectr — do not edit
...
`;
  ...
  return `# Generated by rflectr — do not edit
${profileSandboxLine()}model = ${tomlString(model)}
model_provider = "rflectr-proxy"
model_catalog_json = ${tomlString(catalogPath)}
${reasoning}
[model_providers.rflectr-proxy]
name = "rflectr"
base_url = ${tomlString(proxyBase)}
env_key = "RFLECTR_CODEX_KEY"
wire_api = "responses"
`;
}
...
export function getFavoritesCatalogPath(): string {
  return join(getRflectrCodexDir(), 'models-favorites.json');
}

export function getFavoritesAppCatalogPath(): string {
  return join(getRflectrCodexDir(), 'app-models-favorites.json');
}
