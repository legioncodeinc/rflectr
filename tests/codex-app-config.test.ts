...
describe('app-config', () => {
  ...
  it('patches config and marks app-managed', () => {
    ...
    const catalog = join(home, '.rflectr', 'codex', 'app-models-anthropic.json');
  ...
  it('preserves openai_base_url when restoring a legacy snapshot', () => {
    const configPath = join(home, '.codex', 'config.toml');
    const managed = [
      'model = "rflectr-launch-codex-app/claude-sonnet-4-6"',
      'model_provider = "rflectr-launch-codex-app"',
      'model_catalog_json = "/tmp/app-models-anthropic.json"',
      'openai_base_url = "https://example.test/v1"',
      '',
      '[model_providers.rflectr-launch-codex-app]',
      'name = "rflectr"',
