...
describe('startCodexProxy', () => {
  ...
  it('falls back to first route for unknown model', async () => {
    ...
    const res = await fetch(`http://127.0.0.1:${handle.port}/v1/responses`, {
      ...
      body: JSON.stringify({ model: 'rflectr-launch-codex-app/unknown-model', input: 'hi', stream: false }),
    });
  ...
  it('resolves namespaced catalog model ids', async () => {
    ...
    const route = findCodexProxyRoute(routes, 'rflectr-launch-codex-app/claude-sonnet-4-6');
...
describe('buildCodexProfileToml', () => {
  it('writes proxy tier profile with RFLECTR_CODEX_KEY', async () => {
    ...
    expect(toml).toContain('model_provider = "rflectr-proxy"');
    expect(toml).toContain('sandbox = "danger-full-access"');
    expect(toml).toContain('env_key = "RFLECTR_CODEX_KEY"');
  ...
  it('escapes Windows paths as valid TOML strings', async () => {
    ...
    const toml = buildCodexProfileToml({
      ...
      catalogPath: 'C:\\Users\\legioncodeinc\\rflectr\\models-openai.json',
    });
    const parsed = parse(toml) as {
      model_catalog_json?: string;
      model_providers?: Record<string, { base_url?: string }>;
    };
    expect(parsed.model_catalog_json).toBe('C:\\Users\\legioncodeinc\\rflectr\\models-openai.json');
