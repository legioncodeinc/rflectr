...
import { ownedOverlayPaths, getRflectrCodexDir } from '../src/codex/session.js';
...
beforeEach(() => {
  ...
  previousRelayHome = process.env['RFLECTR_HOME'];
  process.env['HOME'] = tempHome;
  process.env['RFLECTR_HOME'] = join(tempHome, 'rflectr');
});
...
afterEach(() => {
  ...
  if (previousRelayHome === undefined) delete process.env['RFLECTR_HOME'];
  else process.env['RFLECTR_HOME'] = previousRelayHome;
});
...
describe('codex favorites launch config paths', () => {
  it('getFavoritesCatalogPath is matched by --restore glob', () => {
    ...
    mkdirSync(getRflectrCodexDir(), { recursive: true });
    writeFileSync(path, '{}');

    expect(ownedOverlayPaths()).toEqual(expect.arrayContaining([expect.stringMatching(/models-favorites\.json/)]));
  });

  it('buildCodexChildEnv injects RFLECTR_CODEX_KEY=proxy-local for multi-route proxy', () => {
    ...
    expect(env.RFLECTR_CODEX_KEY).toBe('proxy-local');
  });
