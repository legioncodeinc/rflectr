...
describe('vertex-config', () => {
  ...
  beforeEach(() => {
    tempHome = mkdtempSync(join(tmpdir(), 'rflectr-vertex-'));
  });
  ...
  it('uses public-safe default models when no override file exists', () => {
    const env = { RFLECTR_HOME: tempHome };
  ...
  it('loads optional user catalog override from app home', () => {
    ...
    const models = loadVertexModelEntries({ RFLECTR_HOME: tempHome });
  ...
  it('returns null when project env is missing', () => {
    expect(buildVertexRuntimeConfig({ RFLECTR_HOME: tempHome })).toBeNull();
  });
