...
describe('codex app session', () => {
  ...
  beforeEach(() => {
    ...
    prevRelayHome = process.env.RFLECTR_HOME;
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.RFLECTR_HOME = join(home, '.rflectr');
  });
  ...
  afterEach(() => {
    ...
    if (prevRelayHome === undefined) delete process.env.RFLECTR_HOME;
    else process.env.RFLECTR_HOME = prevRelayHome;
  });
  ...
  it('allows the owning rflectr process to restore its own app session', () => {
    const configPath = getCodexConfigPath();
    mkdirSync(join(home, '.codex'), { recursive: true });
    writeFileSync(configPath, 'model = "gpt-5"\nmodel_provider = "openai"\n', 'utf8');

    saveAppRestoreStateBeforePatch();
    const catalogPath = join(home, '.rflectr', 'codex', 'app-models-anthropic.json');
